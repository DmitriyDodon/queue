...
class VisitController
{
    public function getData(GeoServiceInteface $geo , UserAgentInterface $userAgent , Request $request)
    {
        ...
        dispatch(new \App\Jobs\ParserJob(['user_agent' => $request->userAgent() , 'user_ip' => $request->server('HTTP_X_FORWARDED_FOR')]))->onQueue('parsing');
        $request->session()->put([
            'message' => 'Your ip was added to the queue.',
        ]);
